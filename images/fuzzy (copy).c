...
int main(int argc, char **argv){
	...
	int i, j, k, pixel_val, accuracy, faulty, gm, faulty_gm, xx, csf , yy, faulty_csf, zz, wm, faulty_wm;
	...
	if(argc > 2){
		...
		k = 0; accuracy = faulty = gm = faulty_gm = xx = csf = yy = faulty_csf = zz = wm = faulty_wm = 0;
		for(i = 0; i < col; i++)
			for(j = 0; j < row; j++)
				{
		...
		printf("\nGrey Matter Accuracy: %2f\n", (((float)gm*100)/((float)gm+faulty_gm)));
		printf("CSF Accuracy: %2f\n", (((float)csf*100)/((float)csf+faulty_csf)));
		printf("White Matter Accuracy: %2f\n", (((float)wm*100)/((float)wm+faulty_wm)));
	}
